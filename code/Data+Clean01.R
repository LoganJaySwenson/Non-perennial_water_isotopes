# Data + Clean- AIMS Isotopes
library(lubridate)
library(tidyverse)

# Read: AIMS isotopes 
AIMS_isotopes <- read_csv("data/AIMS/AIMS_isotopes.csv") %>%
...
# Read: Isotopes notes
notes <- read_csv("data/AIMS/AIMS_isotopes_notes.csv") %>%
...
AIMS_isotopes$id <- NULL
AIMS_isotopes_notes$id <- NULL

# Save to csv for spatial analysis
write_csv(AIMS_isotopes, "data/AIMS_isotopes_coordinates.csv")

# Save to csv for RF model
write_csv(AIMS_isotopes_notes, "data/AIMS_isotopes_RF.csv")